...
def sparse_constraint(abundance):
    sparse_loss = tf.math.reduce_sum(tf.square(abundance))
    lambda_value = 0.0001
    sparse_loss = lambda_value * sparse_loss
    return sparse_loss
...
class Autoencoder(object):
    def __init__(self, patches, params):
        ...
        self.model.compile(optimizer=self.params["optimizer"], loss=self.compute_loss)
    ...
    def compute_loss(self,y_true,y_pred):
        ...
        loss = SAD(y_true,y_pred)
        sparse_loss = sparse_constraint(abundances)
        # loss = recon_loss+sparse_loss * 0.005
        return loss
